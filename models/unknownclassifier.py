...
def make_history_plot(history: callbacks.History, filepath: Path, epochs: int, title: str):
...
def get_datasets(data_path: str | Path, validation_split: float = 0.2, image_size: tuple = (96, 96)) -> tuple:
...
def fit_model(model: keras.models.Model, training_data: keras.utils.image_dataset, validation_data: keras.utils.image_dataset, epochs: int = 15):
    history = model.fit(training_data,
                        ...
                        epochs=epochs)
    model.save(SAVED_MODEL_PATH.joinpath('unknown_object_identifier.h5'))
    return history
...
def get_new_filename(filepath: Path, filename: str):
    file_count = 1
    for child in filepath.rglob('*'):
        if child.name.lower() == filename.lower() and child.is_file():
            file_count += 1
    return Path(f"{filename}_{file_count}")


if __name__ == '__main__':
    # Set constants
    PLOTTING_PATH = Path.cwd().joinpath("plots")
    PLOTTING_FILENAME = get_new_filename(PLOTTING_PATH, "training_plot")
    DATASET_PATH = Path.cwd().joinpath('models', 'data', 'unknown_classification')
    TEST_PATH = Path.cwd().joinpath('models', 'data', 'uc_test')
    IMG_HEIGHT, IMG_WIDTH = 224, 224
    EPOCHS = 100
    # Pre-process images in dataset directory
    convert_images_in(DATASET_PATH)
    # Get classes and datasets
    classes = get_classes_in(DATASET_PATH)
    train, val = get_datasets(DATASET_PATH, image_size=(IMG_HEIGHT, IMG_WIDTH))
    # Create and train model
    new_model = get_model(1, IMG_HEIGHT, IMG_WIDTH)
    model_history = fit_model(new_model, train, val, epochs=EPOCHS)
    # Plot accuracy and loss
    make_history_plot(model_history, PLOTTING_FILENAME, EPOCHS, "Training 1")
